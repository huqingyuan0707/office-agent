...
- [x] PRD §2.3 审批流程智能助手补齐三缺口（2026-09-25）：①`office.approval.submit`（approval_submit.py，office:write 恒送审 + idem_key 必填≥8 字符，批准后 handler 才落本地台账 DOCS_DIR/data/approvals_ledger.json，读写锁仿 affairs；缺必填执行期诚实 1001 拒收不落残缺台账；(tenant,idem_key) 幂等回放 replayed=True 绝不双单，与联动回流 ticket.create 同口径）②`office.approval.opinion`（approval_opinion.py，office:read 免审——申请人审批说明/复核人意见三档 stance=approve/reject/supplement，reject 必须附理由、supplement 必须真有缺项；只引用入参原文零编造，金额分级阈值 import approval 唯一出处；_amount_hint 取**原始入参**取金额——字符串化后的 filled 过不了 to_amount）③`POST /approvals/{id}/urge` 一键催办（api/approvals.py 薄端点 + services/approval_flow.urge_approval——本人或管理员对 pending 单发一次 IM 提醒，旁路不进 linkage/不改状态/不落库，已决单 4004 拒催，IM 未配置如实 not_configured；im_notifier 新增 KIND_APPROVAL_URGE + notify_approval_urge）。④seed 复核员角色改 admin+approver+**office:read**（旧注释写 approver+viewer 与实际不符已修；不加 office:read 复核员用不动 draft/check/opinion 辅助工具，写口径仍不给）。验证：五包全量 **253 passed**（tools-office +9 test_approval_flow / server +2 催办：本人催+IM 如实降级、越权 1001+管理员代催+已决 4004+不存在 404；催办用例收尾批准防共享库 pending 连坐超时扫描计数）；ruff/naming/arch/alembic check 全绿；HTTP 冒烟 `tests/smoke_approval_flow.py` 双跑 10/10；README 中英 V1.1 审批辅助行扩 opinion/submit + 新增催办行。
...
- [x] PRD §2.5 会议智能协作补齐三缺口（2026-09-25）：`packages/tools-office/office_agent_tools_office/meeting_flow.py` 三只读免审工具——①`office.meeting.materials`（会前整理会议资料：DOCS_DIR 指定文件逐份真实抽取汇编资料包，复用 file_read.extract_document 同一口径，单文件缺失/超限/解析失败如实标 failed 不炸整包，全失败 degraded 不编造）②`office.meeting.digest`（会中梳理要点：速记/转录文本按关键词规则归类决议/行动/风险三类，原句摘录不改写，风险共用 meeting.RISK_KEYWORDS 词表已转公开；实时语音转录需音频基建如实后置）③`office.meeting.followup`（会后落实跟进：纪要行动项×待办台账 affairs 存储按标题精确对账，五态 已完成/进行中/已逾期/已取消/未建单，逾期按业务时区当天判定，查无即未建单不臆造进展）。验证：tools-office 单测 +12 例（test_meeting_flow.py），ruff/naming/arch 全绿，五包 pytest 全量通过（并发方在途 data_analysis.py 的 csv NameError 与本单无关，如实标注）；HTTP 冒烟 `tests/smoke_meeting_flow.py` 双跑 5/5（资料包真实抽取/缺失不炸包/三类归类/建单→完成→对账三态）；README 中英 V1.1 表新增「会议全流程补充」行 + 本文件 §5 冒烟命令登记。
