// 职责：前端路由表与全局守卫（登录态闸门）——业务路由由 router/menu.ts 各域模块聚合，本文件只装配
// 链路：main.ts use(router) → beforeEach 校验 token → MainLayout 壳内懒加载对应视图；/ 重定向 /chat
// 对齐：AGENTS.md §4 前端红线（本文件只做入口闸门，不发接口请求；401 跳转由 api.ts 回调 main.ts 完成）
// 说明：createWebHashHistory —— hash 路由，静态托管零依赖 SPA fallback（无需服务端 rewrite 规则）
import { createRouter, createWebHashHistory, type RouteRecordRaw } from 'vue-router'
import { getToken } from '../api'
import { menuRoutes } from './menu'

const routes: RouteRecordRaw[] = [
  {
    path: '/login',
    component: () => import('../views/LoginView.vue'),
    meta: { public: true }, // 公开页：无 token 可进、不进侧边栏壳（整屏登录卡）
  },
  {
    path: '/',
    component: () => import('../layouts/MainLayout.vue'),
    redirect: '/chat',
    children: menuRoutes, // 菜单即路由：新增页面只改 router/modules/ 对应域模块
  },
]

const router = createRouter({
  history: createWebHashHistory(),
  routes,
})

// 全局守卫：无 token 且非公开页 → /login；已有 token 访问 /login → /chat
router.beforeEach((to) => {
  if (to.path === '/login') return getToken() ? '/chat' : true
  if (!to.meta.public && !getToken()) return '/login'
  return true
})

export default router
