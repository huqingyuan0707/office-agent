...
| Admin & logistics tickets (PRD §2.11) | `office.desk.ticket` / `tickets` | IT repair / asset claim / work order allowlist; ticket is an approved write landing in the local ledger, tickets is an approval-free read; syncing to external systems stays with linkage; new `/desk` page (list + approval-id hint on filing); scene chaining and visual orchestration live under workflows (`/workflows`); fine-grained per-tool permissions already exist as tool scopes + agent whitelists |
| Scheduled jobs (PRD §2.11) | `GET/POST/PUT/DELETE /jobs` + `/jobs/{id}/run` + `/jobs/tick` | Built-in asyncio scheduler loop (`SCHEDULER_ENABLED` off by default, `SCHEDULER_TICK_SECONDS` configurable); job allowlist notify_scan (idempotent notification scan, never spams) / workflow_run (sequential orchestration, write steps still stop at the approval gate); daily/weekly resolved in the business timezone, interval floor 30s; execution identity = creator's live roles (missing/frozen honestly failed); run-now never advances the schedule; with the loop off, external cron can call `/jobs/tick`; the `/jobs` page is fully wired (list/create/toggle/run/tick) |
...
HTTP-level smoke: `python tests/smoke_jobs.py` (§2.11 scheduled jobs, 9 assertions, self-started service with the real scheduler loop firing on time, re-runnable).
